...
const CONTRACT_CONTENT_TYPES: Record<string, string> = {
  pdf: "application/pdf",
...
export async function GET() {
  try {
    ...
    if (!canManageContracts(role))
      return NextResponse.json(
        { error: "This area is for Manager, HR and Admin only." },
        { status: 403 },
      );
    ...
    return NextResponse.json(
      { error: safeApiError(error, "Unable to load contracts.") },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!employeeId || !(file instanceof File) || !file.name) {
      return NextResponse.json(
        { error: "Select an employee and contract file." },
        { status: 400 },
      );
    }

    const extension = file.name.split(".").pop()?.toLowerCase() || "";
    const contentType = CONTRACT_CONTENT_TYPES[extension];
    if (!contentType) {
      return NextResponse.json(
        { error: "Contract must be a PDF, DOC or DOCX file." },
        { status: 400 },
      );
    }
    if (file.size <= 0 || file.size > MAX_UPLOAD_BYTES) {
      return NextResponse.json(
        { error: "Contract must be 10 MB or smaller." },
        { status: 400 },
      );
    }
    ...
    const admin = createSupabaseAdminClient();
    if (role === "manager") {
      const { data: managedEmployee, error: managedEmployeeError } = await admin
        .from("employees")
        .select("id")
        .eq("id", employeeId)
      ...
      if (!managedEmployee)
        return NextResponse.json(
          {
            error:
              "Managers can only upload contracts for employees assigned to their team.",
          },
          { status: 403 },
        );
    }
    ...
    return NextResponse.json(
      {
        message:
          "Contract uploaded. It is now waiting for the employee to sign.",
      },
      { status: 201 },
    );
  } catch (error) {
