...
export default function HrArchivePage() {
  ...
  async function archive(row:ArchiveEmployee){
    ...
    const resuming=Boolean(row.archive_pending_at)&&!alreadyArchived;
    const confirmation=window.prompt(resuming
      ? `A previous archive attempt did not finish. Confirm the employee pack is safely stored on your secure server, then retry the removal.\n\nType ARCHIVE to continue.`
      : alreadyArchived
    ...
    try{
      ...
      const data=await response.json();
      if(!response.ok){setMessage(data.error||'The employee record could not be archived.');await load();return}
      setMessage(`${row.name}: ${data.message}`);
      await load();
    }catch{setMessage('The employee record could not be archived. Please retry from this page.');await load()}finally{setWorking(null)}
  }

  return <main className="shell"><Header/><section className="hero"><span className="pill">HR only</span><h1>HR Archive</h1><p className="muted">Use this only after the employee pack has been downloaded and saved to your secure server. The portal keeps only minimum audit evidence after archiving.</p></section>{message&&<section className="section"><div className="card" role="status"><strong>{message}</strong></div></section>}<section className="section"><div className="card"><h2>Employee records</h2>{loading?<p>Loading...</p>:<div style={{overflowX:'auto'}}><table className="admin-table"><thead><tr><th>Employee</th><th>Status</th><th>Submitted</th><th>Signed files to archive</th><th>Awaiting signature</th><th>Action</th></tr></thead><tbody>{rows.map(row=>{const pending=Boolean(row.archive_pending_at)&&!row.archived_at;return <tr key={row.employee_id}><td><strong>{row.name}</strong><br/><span className="muted">{row.email||''}</span></td><td>{row.archived_at?'Archived':pending?'Archive pending':row.status}</td><td>{row.submitted_at?new Date(row.submitted_at).toLocaleString():'—'}</td><td>{row.signed_contracts}</td><td>{row.awaiting_contracts}</td><td>{row.awaiting_contracts>0?<span className="warn">Complete contract signing first</span>:row.archived_at&&row.signed_contracts===0?<span className="ok">Private portal data removed</span>:pending?<button className="button" type="button" disabled={working===row.employee_id} onClick={()=>archive(row)}>{working===row.employee_id?'Retrying...':'Retry archive'}</button>:row.status!=='submitted'&&!row.archived_at?<span className="muted">Submit onboarding first</span>:<button className="button" type="button" disabled={working===row.employee_id} onClick={()=>archive(row)}>{working===row.employee_id?'Archiving...':row.archived_at?'Archive new contract files':'Archive & remove private data'}</button>}</td></tr>})}</tbody></table></div>}</div></section><section className="section"><p className="muted">Export the onboarding PDF from HR Admin and download signed contracts before using the archive action.</p><Link href="/admin">← Back to HR Admin</Link></section></main>;
}
