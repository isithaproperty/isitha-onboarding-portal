...
export default function HrArchivePage() {
  ...
  return <main className="shell"><Header/><section className="hero"><span className="pill">HR only</span><h1>HR Archive</h1><p className="muted">Use this only after the employee pack has been downloaded and saved to your secure server. The portal keeps only minimum audit evidence after archiving.</p></section>{message&&<section className="section"><div className="card" role="status"><strong>{message}</strong></div></section>}<section className="section"><div className="card"><h2>Employee records</h2>{loading?<p>Loading...</p>:<div style={{overflowX:'auto'}}><table className="admin-table"><thead><tr><th>Employee</th><th>Status</th><th>Submitted</th><th>Signed contracts</th><th>Awaiting signature</th><th>Action</th></tr></thead><tbody>{rows.map(row=><tr key={row.employee_id}><td><strong>{row.name}</strong><br/><span className="muted">{row.email||''}</span></td><td>{row.archived_at?'Archived':row.status}</td><td>{row.submitted_at?new Date(row.submitted_at).toLocaleString():'—'}</td><td>{row.signed_contracts}</td><td>{row.awaiting_contracts}</td><td>{row.archived_at?<span className="ok">Private portal data removed</span>:row.status!=='submitted'?<span className="muted">Submit onboarding first</span>:<button className="button" type="button" disabled={working===row.employee_id||row.awaiting_contracts>0} onClick={()=>archive(row)}>{working===row.employee_id?'Archiving...':'Archive & remove private data'}</button>}</td></tr>)}</tbody></table></div>}</div></section><section className="section"><Link href="/admin">← Back to HR Admin</Link></section></main>;
}
