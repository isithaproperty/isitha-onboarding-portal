...
export default function TeamLeave(){
 ...
 useEffect(()=>{load()},[]);
 async function load(){const {data:{user}}=await supabase.auth.getUser();if(!user){router.push('/login');return;} const role=String(user.app_metadata?.role||'staff').toLowerCase(); if(!['manager','hr','hr_admin','admin','administrator'].includes(role)){setLoading(false);return;} setAllowed(true); const {data,error}=await supabase.from('leave_requests').select('id,employee_id,leave_type,start_date,end_date,reason,status,manager_comment,employees(first_name,last_name)').order('created_at',{ascending:false}); if(error){console.error(error);setRows([]);}else{setRows((data||[]) as unknown as Row[]);}setLoading(false);}
